...
import { API_PROJECTS_SUBUNIDAD } from "@/config/apiconfig"
import {
  Card,
  CardContent,
  CardDescription,
...
const chartData = [
  { date: "2022-07-01", completado:1,pendiente: 222, archivado: 150  , curso: 400 },
...
export default function Component() {
  ...
  const [chartData1, setChartData1] = React.useState(chartData)
  ...
  const { idrol, idsubuni, dni } = useParams();
    try {
      const response = await fetch(`${API_PROJECTS_SUBUNIDAD}/${idrol}`);
      if (!response.ok) {
